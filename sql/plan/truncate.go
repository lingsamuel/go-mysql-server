...
// Child returns the referenced table.
func (p *Truncate) Child() sql.Node {
	return p.tbl
}

// Children implements the Node interface.
func (p *Truncate) Children() []sql.Node {
	return []sql.Node{p.tbl}
}
...
func (p *Truncate) Resolved() bool {
	return p.tbl.Resolved()
}

// RowIter implements the Node interface.
func (p *Truncate) RowIter(ctx *sql.Context, _ sql.Row) (sql.RowIter, error) {
	truncatable, err := GetTruncatable(p.tbl)
	if err != nil {
		return nil, err
	}
...
func (p Truncate) String() string {
	...
	_ = pr.WriteChildren(p.tbl.String())
	return pr.String()
}
...
func (p Truncate) DebugString() string {
	pr := sql.NewTreePrinter()
	_ = pr.WriteNode("Truncate")
	_ = pr.WriteChildren(sql.DebugString(p.tbl))
	return pr.String()
}
