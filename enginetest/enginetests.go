...
func TestTruncate(t *testing.T, harness Harness) {
	...
	t.Run("Standard TRUNCATE", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t1 (pk BIGINT PRIMARY KEY, v1 BIGINT, INDEX(v1))")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t1 VALUES (1,1), (2,2), (3,3)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		...
		_, iter, err = e.Query(ctx, "INSERT INTO t1 VALUES (4,4), (5,5)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
	...
	t.Run("Foreign Key References", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t2parent (pk BIGINT PRIMARY KEY, v1 BIGINT, INDEX (v1))")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "CREATE TABLE t2child (pk BIGINT PRIMARY KEY, v1 BIGINT, "+
	...
	t.Run("ON DELETE Triggers", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t3 (pk BIGINT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "CREATE TABLE t3i (pk BIGINT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "CREATE TRIGGER trig_t3 BEFORE DELETE ON t3 FOR EACH ROW INSERT INTO t3i VALUES (old.pk, old.v1)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t3 VALUES (1,1), (3,3)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
	...
	t.Run("auto_increment column", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t4 (pk BIGINT AUTO_INCREMENT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t4(v1) VALUES (5), (6)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
		...
		_, iter, err = e.Query(ctx, "INSERT INTO t4(v1) VALUES (7)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
	...
	t.Run("Naked DELETE", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t5 (pk BIGINT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t5 VALUES (1,1), (2,2)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
		...
		require.NoError(t, err)
		truncateFound := false
		sql.InspectNode(analyzed, func(n sql.Node) bool {
			switch n.(type) {
			case *plan.Truncate:
				truncateFound = true
				return false
	...
	t.Run("Naked DELETE with Foreign Key References", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t6parent (pk BIGINT PRIMARY KEY, v1 BIGINT, INDEX (v1))")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "CREATE TABLE t6child (pk BIGINT PRIMARY KEY, v1 BIGINT, "+
			"FOREIGN KEY (v1) REFERENCES t6parent (v1))")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t6parent VALUES (1,1), (2,2)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t6child VALUES (1,1), (2,2)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
	...
	t.Run("Naked DELETE with ON DELETE Triggers", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t7 (pk BIGINT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "CREATE TABLE t7i (pk BIGINT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "CREATE TRIGGER trig_t7 BEFORE DELETE ON t7 FOR EACH ROW INSERT INTO t7i VALUES (old.pk, old.v1)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t7 VALUES (1,1), (3,3)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "DELETE FROM t7 WHERE pk = 3")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
	...
	t.Run("Naked DELETE with auto_increment column", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t8 (pk BIGINT AUTO_INCREMENT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t8(v1) VALUES (4), (5)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
		...
		_, iter, err = e.Query(ctx, "INSERT INTO t8(v1) VALUES (6)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
	...
	t.Run("DELETE with WHERE clause", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t9 (pk BIGINT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t9 VALUES (7,7), (8,8)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
	...
	t.Run("DELETE with LIMIT clause", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t10 (pk BIGINT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t10 VALUES (8,8), (9,9)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
	...
	t.Run("DELETE with ORDER BY clause", func(t *testing.T) {
		_, iter, err := e.Query(ctx, "CREATE TABLE t11 (pk BIGINT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t11 VALUES (1,1), (9,9)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
	...
	t.Run("Multi-table DELETE", func(t *testing.T) {
		...
		_, iter, err := e.Query(ctx, "CREATE TABLE t12a (pk BIGINT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "CREATE TABLE t12b (pk BIGINT PRIMARY KEY, v1 BIGINT)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t12a VALUES (1,1), (2,2)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		_, iter, err = e.Query(ctx, "INSERT INTO t12b VALUES (1,1), (2,2)")
		require.NoError(t, err)
		_, err = sql.RowIterToRows(iter)
		require.NoError(t, err)
		TestQuery(t, harness, e,
